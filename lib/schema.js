...
function read(database, username, password, options, namingOptions, callback) {
    ...
    sequelize
        .query(sql)
        .then(function (rows) { return processTablesAndColumns(undefined, rows); })
    ...
    function readReferences() {
        ...
        sequelize
            .query(sql)
            .then(function (rows) { return processReferences(undefined, rows); })
    ...
    function processReferences(err, rows) {
        ...
        function processReferenceRow(row) {
            ...
            childTable.fields.push(new Field(util.camelCase(Sequelize.Utils.singularize(associationName === undefined
                ...
                : associationName)), ChangeCase[naming.defaults.caseType](ChangeCase.snake(row.referenced_table_name) + '_pojo'), undefined, undefined, childTable, true));
    ...
    function addViewReference(view, index, array) {
        ...
        function addViewFieldReference(field, index, array) {
            ...
            otherTable.fields.push(new Field(util.camelCase(view.tableName), Sequelize.Utils.singularize(view.tableName) + 'Pojo[]', undefined, undefined, otherTable, true));
        }
    }
