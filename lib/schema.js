...
exports.Table = Table;
var Field = (function () {
    function Field(fieldName, fieldType, columnType, isNullableString, table, isReference, isCalculated) {
...
function read(database, username, password, options, callback) {
    ...
    var idFieldLookup = {};
    var sql = "select table_name, column_name, is_nullable, data_type, column_type, ordinal_position " +
    ...
    function readCustomFields(originalRows) {
        ...
        var sql = "select table_name, column_name, is_nullable, data_type, column_type, referenced_table_name, referenced_column_name, ordinal_position " +
    ...
    function processTablesAndColumnsWithCustom(rows, customFieldLookup) {
        ...
        for (var index = 0; index < rows.length; index++) {
            ...
            var field = new Field(row.column_name, row.data_type, row.column_type, row.is_nullable, table, false, isCalculated);
    ...
    function processReferences(err, rows) {
        ...
        function processReferenceRow(row) {
            ...
            if (!row.hasOwnProperty('ordinal_position')) {
                parentTable.fields.push(new Field(util.camelCase(row.table_name), Sequelize.Utils.singularize(row.table_name) + 'Pojo[]', undefined, undefined, parentTable, true));
            }
            childTable.fields.push(new Field(util.camelCase(Sequelize.Utils.singularize(associationName === undefined
                ? row.referenced_table_name
                : associationName)), Sequelize.Utils.singularize(row.referenced_table_name) + 'Pojo', undefined, undefined, childTable, true));
        ...
        function processReferenceXrefs() {
            for (var xrefName in xrefs) {
                ...
                firstTable.fields.push(new Field(util.camelCase(xref.secondTableName), Sequelize.Utils.singularize(xref.secondTableName) + 'Pojo[]', undefined, undefined, firstTable, true));
                secondTable.fields.push(new Field(util.camelCase(xref.firstTableName), Sequelize.Utils.singularize(xref.firstTableName) + 'Pojo[]', undefined, undefined, secondTable, true));
            }
        }
    }
    ...
    function addViewReference(view, index, array) {
        ...
        function addViewFieldReference(field, index, array) {
            ...
            view.fields.push(new Field(otherTableSingular, otherTableSingular + 'Pojo', undefined, undefined, view, true));
            otherTable.fields.push(new Field(util.camelCase(view.tableName), Sequelize.Utils.singularize(view.tableName, 'en') + 'Pojo[]', undefined, undefined, otherTable, true));
        }
    }
